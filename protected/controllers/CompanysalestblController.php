...
class CompanysalestblController extends Controller
{
	/**
	...
	{
		$model=new Companysalestbl;

		// Uncomment the following line if AJAX validation is needed
		// $this->performAjaxValidation($model);

		if(isset($_POST['Companysalestbl']))
		{
	...
	public function actionUpdate($id)
	{
		...
		if(isset($_POST['Companysalestbl']))
		{
                      ...
                      if(!empty($_FILES['Companysalestbl']["tmp_name"]["image"]))
                      {
                          $rnd =time().rand(0,9999);    
                          $model->image  = $rnd.$_FILES['Companysalestbl']["name"]["image"];
                          move_uploaded_file($_FILES['Companysalestbl']["tmp_name"]["image"], "./test/".$rnd.$_FILES['Companysalestbl']["name"]["image"]);
                      }
                      else 
                      {
	...
	{
		$dataProvider=new CActiveDataProvider('Companysalestbl');
	...
	{
		$model=new Companysalestbl('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['Companysalestbl']))
			$model->attributes=$_GET['Companysalestbl'];
	 ...
	 * @return Companysalestbl the loaded model
	 * @throws CHttpException
	 */
	public function loadModel($id)
	{
		$model=Companysalestbl::model()->findByPk($id);
		if($model===null)
	 ...
	 * @param Companysalestbl $model the model to be validated
	 */
	protected function performAjaxValidation($model)
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='companysalestbl-form')
		{
