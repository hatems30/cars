...
class UserpertblController extends Controller
{
	/**
	...
	{
		$model=new Userpertbl;

		// Uncomment the following line if AJAX validation is needed
		// $this->performAjaxValidation($model);

		if(isset($_POST['Userpertbl']))
		{
	...
	{
		$dataProvider=new CActiveDataProvider('Userpertbl');
	...
	{
		$model=new Userpertbl('search');
              //  $model=Userpertbl::model()->findAllByAttributes(array('user_id'=>1));
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['Userpertbl']))
	        $model->attributes=$_GET['Userpertbl'];
              //$model=Userpertbl::model()->findAllByAttributes(array('user_id'=>1));
	 ...
	 * @return Userpertbl the loaded model
	 * @throws CHttpException
	 */
	public function loadModel($id)
	{
		$model=Userpertbl::model()->findByPk($id);
		if($model===null)
	 ...
	 * @param Userpertbl $model the model to be validated
	 */
	protected function performAjaxValidation($model)
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='userpertbl-form')
		{
