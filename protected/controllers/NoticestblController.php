...
class NoticestblController extends Controller
{
	/**
	...
	{
		$model=new Noticestbl;
		if(isset($_POST['Noticestbl']))
		{
                        $rnd =time().rand(0,9999);                       //customer image
			$model->attributes=$_POST['Noticestbl'];
	...
	public function actionUpdate($id)
	{
		...
		if(isset($_POST['Noticestbl']))
		{
	              ...
	              $model->attributes=$_POST['Noticestbl'];  
                      ...
                      if(!empty($_FILES['Noticestbl']["tmp_name"]["image"]))
                      {
                          $rnd =time().rand(0,9999);    
                          $model->image  = $rnd.$_FILES['Noticestbl']["name"]["image"];
                          move_uploaded_file($_FILES['Noticestbl']["tmp_name"]["image"], "./test/".$rnd.$_FILES['Noticestbl']["name"]["image"]);
                      }
                      else 
                      {
	...
	{
		$dataProvider=new CActiveDataProvider('Noticestbl');
	...
	{
		$model=new Noticestbl('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['Noticestbl']))
			$model->attributes=$_GET['Noticestbl'];
	 ...
	 * @return Noticestbl the loaded model
	 * @throws CHttpException
	 */
	public function loadModel($id)
	{
		$model=Noticestbl::model()->findByPk($id);
		if($model===null)
	 ...
	 * @param Noticestbl $model the model to be validated
	 */
	protected function performAjaxValidation($model)
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='noticestbl-form')
		{
